...
class FirstNN(object):
    ...
    def backword(self, X, Y, prob_scores):
        ...
        prob_score_neg_y[(range(N),Y)] -= 1 #NxC

        grads['W2']= np.dot(self.O_1.T, prob_score_neg_y)/N #MxC
        grads['b2']= np.sum(prob_scores, axis=0)/N #C

        dZ_1 =np.dot(prob_score_neg_y, self.params['W2'].T)*(self.O_1*(1-self.O_1)) #NxN
        
        grads['W1']=np.dot(X.T, dZ_1)/N 
        grads['b1'] =np.sum(dZ_1, axis=0)/N
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
